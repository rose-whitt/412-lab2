...
/* refactor this code into a comment parser, a line matcher, and
   an answer checker. */
...
int main( int argc, char **argv ) {
  ...
  while( b_len != (size_t) -1) {
	...
	fprintf(stdout,"Found output spec '%s'\n",output_spec);
      ...
      if (DEBUG>0) 
	fprintf(stdout,"-> '%s' (blank line?).\n",buffer);

      b_len = (int) getline(&buffer,&b_len,stdin);
      buffer[b_len-1] = '\0'; 

      if (DEBUG>0) 
	fprintf(stdout,"-> '%s' (Code Check label?).\n",buffer);

      p = next_number_from_string(output_spec,&expected,"source");
      value = 1;
      correct = 1;
	...
	if (strlen(fname) > 5)
	  fprintf(stdout,"Program '%s'\tcorrectly executed in %d cycles\n",
		  fname,cycles);
	else
	  fprintf(stdout,"Program '%s'\t\tcorrectly executed in %d cycles\n",
		  fname,cycles);
