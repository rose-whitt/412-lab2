...
def run_test(submission):
    ...
    for i in range(0,8):
        if t_cutoff[i] == 0:
            ...
            print("\t"+t_names[i]+":  \ttimed out at 60 seconds.")
    ...
    if language.find("python") > -1:
        ...
        if time <= 4.75:
            ...
            # 0.0725 is (12 - 4.75) / 100
            eff_points = 100 - (time - 2.5) / 0.0725  
    ...
    if noninc_ct > 0:
        scaling += " (?)"
        print("\n\tAnomalous behavior: "+str(noninc_ct)+" inputs showed minimal growth")
...
def main():
    ...
    result_file = open(result_path + 'TimerResults.txt','w')
    ...
    for submission in sorted(os.listdir('./')):
        ...
        print("Testing submission from NetID '"+current_id+"', name '"+current_name+"'")
        print("Archive name:  "+submission+"\t\t"+str(NumberOfTrials)+" trials per block\n")
